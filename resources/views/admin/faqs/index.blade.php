...
<div id="content">
  <div id="content-header">
    <div id="breadcrumb"> <a href="{{ url('/admin/dashboard/') }}" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a> <a href="#" class="current">Faqs Listing</a> </div>
    <h1>Faqs Listing</h1>
  ...
  <div class="container-fluid"><hr>
    <div class="row-fluid">  <form  class="form-horizontal"  enctype="multipart/form-data" method="post" action="{{ url('admin/faqs') }}" name="index" id="memberindex" novalidate="novalidate"> 
        <div class="span12">
         ...
         <input type="hidden" name="_token" value="{{ csrf_token() }}">
			...
			<span >
			<a href="{{ url('/admin/faqs/add/') }}" class="btn btn-mini btn-info top_div pull-right" style="margin-right:8px;" title="Add Sms Page">+ Add</a>
			</span>
            ...
            <table class="table table-bordered table-striped with-check data-table" id="example">
			  ...
			  @foreach($setData as $data)
				...
				<input type="checkbox" value="{{ $data['id'] }}" class="checkbox_align select-checkbox" name="box1[]"  >
                  ...
                  <td class="center">{!! str_limit($data['question'], 100) !!}</td>
                  <td class="center">{!! str_limit($data['answer'],200) !!}</td>
                  <td class="center">
				 ...
				 <td class="center">
											    ...
											    <a type="button" class="btn btn-success btn-mini" title="Edit" href="{{ url('/admin/faqs/edit/'.$data['id']) }}"><i class="fa icon-edit"></i></a>
												  <a type="button" data-toggle="modal" class="btn btn-info btn-mini" title="View" href="#myModal{{ $data['id'] }}"><i class="icon-eye-open"></i></a>
												  <a type="button" class="btn btn-danger btn-mini" title="Edit" href="{{ url('/admin/faqs/delete/'.$data['id']) }}"><i class="fa icon-remove" onclick="return confirm('Are you sure, you want to delete it?')"></i></a>
