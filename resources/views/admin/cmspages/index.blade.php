...
<div id="content">
  <div id="content-header">
    <div id="breadcrumb"> <a href="{{ url('/admin/dashboard/') }}" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a> <a href="#" class="current">Cms Page Listing</a> </div>
    <h1>Cms Page Listing</h1>
  ...
  <div class="container-fluid"><hr>
    <div class="row-fluid">  <form  class="form-horizontal"  enctype="multipart/form-data" method="post" action="{{ url('admin/cmspages') }}" name="index" id="memberindex" novalidate="novalidate"> 
        <div class="span12">
		...
		<span >
			<a href="{{ url('/admin/cmspages/add-cmspage/') }}" class="btn btn-mini btn-info top_div pull-right" style="margin-right:8px;" title="Add CmsPage">+ Add</a>
			</span>
            ...
            <table class="table table-bordered table-striped with-check data-table" id="example">
			  ...
			  @foreach($listData as $data)
				...
				<input type="checkbox" value="{{ $data['id'] }}" class="checkbox_align select-checkbox" name="box1[]"  >
                  ...
                  <td class="center">{{ $data['page_name'] }}</td>
                  <td class="center">{{ $data['slug'] }}</td>
				  <td class="center">{{ $data['keywords'] }}</td>
				 ...
				 <td class="center">
											    ...
											    <a type="button" class="btn btn-success btn-mini" title="Edit" href="{{ url('/admin/cmspages/edit/'.$data['id']) }}"><i class="fa icon-edit"></i></a>
									 ...
									 <div id="myModal{{ $data['id'] }}" class="modal hide">
									  <div class="modal-header">
										...
										<h3>{{ $data['page_name'] }} Full Details</h3>
									  </div>
									  <div class="modal-body">
										<p>Cms Page ID: {{ $data['id'] }}</p>
										<p>Name: {{ $data['page_name'] }}</p>
										<p>Slug : {{ $data['slug'] }}</p>
